...
with zipfile.ZipFile(source,'w',zipfile.ZIP_DEFLATED) as z:
 for p in sorted((r/'kicad').rglob('*')):
  if p.is_file() and p.suffix!='.kicad_prl' and not p.name.endswith('.lck'):z.write(p,Path('kicad')/p.relative_to(r/'kicad'))
...
names=['MA600A-v0.3-Gerbers.zip',source.name,'BOM.csv','Pick-and-place.csv','Pin-verification.csv','Assembly-drawing.pdf','Schematic.pdf','ORDER-INSTRUCTIONS.md','Design-review.md','ERC-review.md','First-article-checklist.md','DRC.txt','Verification.json','Gerber-preview-top.png','Gerber-preview-bottom.png','Paste-preview.png','Mask-openings-preview.png']
