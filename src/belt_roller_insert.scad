  ...
  difference(){
    union(){
      intersection(){
        ...
        union(){
          scale([1,(Belt_roller_containing_cube_ywidth-0.2)/Belt_roller_containing_cube_ywidth, 1])
          belt_roller_containing_cube();
          for(k=[0,1])
            mirror([0,k,0]){
              translate([-Belt_roller_containing_cube_xwidth/2, -(Belt_roller_containing_cube_ywidth-0.2)/2, Belt_roller_h - Belt_roller_insert_h])
                rotate([0,0,40])
        ...
        mirror([0,k,0]) {
          intersection(){
            translate([Belt_roller_top_adj_screw_x, Belt_roller_top_adj_screw_y,Belt_roller_h-Belt_roller_insert_h])
              union() {
                difference() {
                  cylinder(d=M3_screw_head_d-0.25, h=Belt_roller_insert_h, $fn=24);
                  translate([-M3_screw_head_d/2, -M3_screw_head_d-2.1, -1])
                    cube([M3_screw_head_d, M3_screw_head_d, Belt_roller_insert_h+2]);
                }
                translate([0,0,Belt_roller_insert_h-9])
                  cylinder(d2=M3_screw_head_d-0.2, d1=0.1, h=9, $fn=24);
              }
