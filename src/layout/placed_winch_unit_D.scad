      ...
      for(k=[0, 1]) mirror([0,k,0]){
        ...
        if (!twod && k==0 && guy_wires)
          line_from_to([move_AD_deflectors - 7, -Spool_height, hz],
                       [anchors[D][X] - 185 - 65, -anchors[D][Y] - 131, anchors[I][Z] - anchors[D][Z] - 16], twod=false);
        if (!twod && k==1 && guy_wires)
          line_from_to([move_AD_deflectors - 7, -Spool_height, hz],
                       [anchors[D][X] - 186 - 65, -anchors[D][Y] + 169 - Sidelength,  anchors[I][Z] - anchors[D][Z] - 16], twod=false);
      }
  }
}
