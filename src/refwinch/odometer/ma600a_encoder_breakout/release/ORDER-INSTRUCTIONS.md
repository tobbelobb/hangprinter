...
9. If checkout asks for an **EORI number** for ICS2/customs, use the EORI belonging to the legal consignee shown on the commercial invoice. For a Swedish business whose VAT number is `SE` + ten digits + `01`, the likely EORI is `SE` + the same ten digits, without the final `01`. Validate that candidate in the [EU EORI validator](https://ec.europa.eu/taxation_customs/dds2/eos/eori_validation.jsp?Lang=sv). If it is invalid, apply free of charge through [Swedish Customs](https://eoriwebb.tullverket.se/ea-ansokan-klient/sv/ansokan) and wait for the assigned number before entering it. Do not put the full VAT number in the EORI field.
10. Review the final quote for five assembled, depanelized boards, exact BOM parts, finish and shipping. Treat the displayed total as excluding Swedish import VAT, customs duty and carrier clearance fees unless the selected delivery method explicitly says **DDP**, **IOSS**, **tax included** or **duties paid**. Resolve any substitution or manufacturing change before paying and releasing production.
11. On delivery, follow the acceptance tests at the end of this document and `First-article-checklist.md`.
...
The EORI request identifies the EU importer to the carrier and customs authorities; it is not a request to register personally for the ICS2 filing system. Swedish EORI numbers normally contain `SE` followed by the ten-digit organisation number, or the personal identity number for a sole trader. VAT registration does not by itself prove that an EORI is active, so use the validator before submitting the order. Swedish Customs states that most online applications are processed within an hour, although some take up to a week. [Swedish Customs EORI guidance](https://www.tullverket.se/eori)

PCBWay states that duties and taxes are normally not included in its item or shipping prices and remain the recipient's responsibility, except when the chosen delivery service explicitly provides DDP or IOSS handling. For a Swedish VAT-registered business importing for its taxable activity, report the import tax base and output VAT in the VAT return and deduct the eligible input VAT there; keep the customs decision and carrier invoice for the accounts. This normally makes the VAT an accounting item rather than a final product cost, but customs duty and carrier clearance charges remain costs. [PCBWay customs guidance](https://www.pcbway.com/helpcenter/Findproducts/Do_I_have_to_pay_for_customs_and_import_taxes_.html) · [Swedish Tax Agency import-VAT guidance](https://www.skatteverket.se/foretag/moms/kopavarorochtjanster/inkopfranlanderutanforeu/kopavarorfranlanderutanforeu.4.361dc8c15312eff6fd37208.html)
