...
tower_h = higher_bearing_z + b623_big_ugroove_big_r + 4.6;
w = b623_big_ugroove_big_r+bw+1.4;
bearing_1_x = b623_big_ugroove_small_r+w/6-0.8;
s = b623_width + 0.8;
...
l = d + 2*b623_big_ugroove_big_r + 2*bw;
...
module action_point_holes(){
  for(k=[0,1]) mirror([k,0,0])
    translate([2/2+b623_width/2,Move_d_bearings_inwards,0]) {
      translate([0,Cc_action_point_from_mid - 0*b623_big_ugroove_small_r + Eyelet_extra_dist,-1])
        eyelet(h=wth+20);
      translate([0,Cc_action_point_from_mid - 2*b623_big_ugroove_small_r - Eyelet_extra_dist,-1])
        eyelet(h=wth+16);
...
module corner_clamp_tower(base_th       = wth,
  ...
  move_tower_x = 2.0;
  top_off_r = b623_big_ugroove_small_r;
  module wall(w_local        = w,
    ...
    translate([move_tower_x, -lwth-shoulder-bearing_width/2, 0]){
      rotate([-90,-90,0]){
        difference(){
          union(){
            // Foot with a swing
            translate([tower_h-b623_big_ugroove_big_r,w/2,lwth])
              difference(){
                  translate([-tower_h+b623_big_ugroove_big_r, -x_len+4.5, -b_th])
                    if(big_y_r2_local)
                      cube([tower_h-foot_shape_r, l, b_th]);
                    else
                      union(){
                        cube([tower_h-foot_shape_r, l-backit, b_th]);
                        translate([tower_h-foot_shape_r - 1,l-backit-14,0])
                          union(){
                            one_rounded_cube2_4([16,14,b_th], r=7, $fn=4*8);
                            translate([1,0,b_th])
                              rotate([180,0,0])
                                inner_round_corner(r=6, h=b_th, $fn=4*8);
                          }
                      }
                if(big_y_r2_local)
                ...
                translate([-0.1-tower_h+b623_big_ugroove_big_r, -(w+f+2*foot_shape_r+20)/2, -15])
    ...
    translate([move_tower_x,0,0]) {
      ...
      difference() {
        translate(b_pos)
          rotate([-90,270+10,0])
            rotate_extrude(angle=75,convexity=10, $fn=60)
              translate([rot_r,0])
                polygon(points = [[0,0], [0,-0.5], [b+a, -0.5], [b+a,0], [b, a], [0, a]]);
        translate([0, -(bearing_width + 2*wth)/2, 0]){
          rotate([-90,-90,0]){
            translate([0,0,lwth]){
              translate([tower_h-b623_big_ugroove_big_r,w/2,0]){
                if(big_y_r2_local)
    ...
    translate([d/2,20,0])
      rotate([0,0,-90])
        difference(){
          ...
          translate([0,0, lower_bearing_z - b623_big_ugroove_small_r - Eyelet_extra_dist])
            rotate([0,90,0])
              eyelet(h=5, center=true);
          translate([0,0, higher_bearing_z + b623_big_ugroove_small_r + Eyelet_extra_dist])
            rotate([0,90,0])
              eyelet(h=5, center=true);
...
module corner_clamp(){
  ...
  channel_l = Cc_action_point_from_mid-14.5;
  channel_r1 = 1;
  channel_r2 = 5;
    ...
    union(){
      ...
      difference(){
        ...
        translate([-(channel_r2+6)/2,channel_l,wth+3])
          rotate([90-atan((Fat_beam_width-2.5)/channel_l),0,0])
  ...
  difference(){
    union(){
      ...
      translate([0,Cc_action_point_from_mid,higher_bearing_z-10])
        rotate([-110,0,0])
          translate([0,0,7.8])
            cylinder(d=7, h=5.5, center=true, $fn=12*2);
      translate([0,Cc_action_point_from_mid,higher_bearing_z+24.7])
        rotate([-110,0,0])
          translate([0,0,7.9])
            cylinder(d=7, h=4.3, center=true, $fn=12*2);
    }
    ...
    //                rotate([90,0,0])
    //                  cylinder(r=b623_big_ugroove_big_r+1.5, h=b623_width, center=true);
